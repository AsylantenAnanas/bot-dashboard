...
class BotManager extends EventEmitter {
  ...
  sendChatMessage(clientId, message) {
    ...
    if (botData && botData.status === 'running') {
      const chatMessage = {
        timestamp: new Date().toISOString(),
        text: `> ${message}`  // So sieht man, dass der Bot diese Nachricht sendet
      };
  ...
  exportAllLogs() {
    let htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          /* Add any additional Minecraft-like styles here */
        </style>
      </head>
      <body>
    ...
    for (const clientId in this.bots) {
      ...
      botObj.messages.forEach((msg) => {
        ...
        const messageHtml = ansiToHtml(msg.text);
